...
module Seven_Seg(
    ...
    reg seven_seg_enable = 1'b0;    // 1-bit enable signal for 1kHz multiplexing speed of 7-segments
     ...
     always @(posedge clk) begin
         ...
         // if(count_fast == 17'd100) begin  // simulation
     ...
     always @(posedge clk) begin
      ...
      always @ (*) begin                       
        case (anode)    // assigning the 32-bit data_disp to each 7-segment unit
            ...
            8'b11111011 : begin 
                data_disp = data[11:8];
                dp = 1'b0;  // turn on the dot point for the 7-segment unit
            end
            8'b11110111 : begin // first point
                data_disp = data[15:12];
                dp = 1'b1; 
            end
            8'b11101111 : begin
                data_disp = data[19:16];
                dp = 1'b0;  // turn on the dot point for the 7-segment unit
            end
            8'b11011111 : begin // second point
                data_disp = data[23:20];
                dp = 1'b1;
            end
            8'b10111111 : begin 
                data_disp = data[27:24];
                dp = 1'b0;  // turn on the dot point for the 7-segment unit
            end
            8'b01111111 : begin // third point
                data_disp = data[31:28];
                dp = 1'b1;
            end
            default : begin
                data_disp = 4'h0;
